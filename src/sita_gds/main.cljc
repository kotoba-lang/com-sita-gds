(ns sita_gds.main
  "Kotodama WASM entrypoint for the Sita_gds clean-room actor (L4) — Clojure port.
  ...
  facts (`sita_gds.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "sita_gds")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"sita_gds.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "sita_gds-compat" :tier tier :entities entities} 200])
...
(defn start! [] :sita_gds-compat/ready)
